...
def main():
    ...
    for ax, tr in zip(axes, TRACKS):
        ...
        sc = ax.scatter(sub[COL_PC1], sub[COL_PC2],
                        ...
                        edgecolor="none")
        ax.set_title(f"PCA • Δ@1", fontsize=12)
    ...
    fig, axes = plt.subplots(nrow, figsize=(5.0, 4.8*nrow), dpi=DPI, constrained_layout=True)
    if nrow == 1:
        axes = np.array([axes])
    ...
    for i, tr in enumerate(TRACKS):
        ...
        sc1 = ax.scatter(sub[COL_TSNE1], sub[COL_TSNE2],
                         ...
                         edgecolor="none")
        ax.set_title(f"t-SNE • Δ@1", fontsize=12)
        ax.set_xlabel("t-SNE1"); ax.set_ylabel("t-SNE2")
        ax.grid(True, linewidth=0.3, alpha=0.5)

    # 共通カラーバーを2本（Δ@1, Δ@10）
    #cax1 = fig.add_axes([0.91, 0.55, 0.015, 0.35])

    cb1 = fig.colorbar(sc1, ax=axes, shrink=0.9)
