...
を用いて、
  ...
  1) LLM 生成（各問題ごとに N_TRIES 回, 4bit量子化, RTX 3060 12GB想定）
  2) ```python``` 抽出 → extracted.py
  3) Docker(extracted-exec:py312)で各入力を実行（各ケースを並列化）
     - timeout, time の出力で実行時間/Max RSS を取得（★1ms精度の計測）
     - Docker --memory により MLE を検出（exit code 137）
  4) 期待出力と照合（数値は誤差許容、regex対応）
  5) verdict.json に WA/TLE/RE/MLE/AC、time、peak_mem、code_bytes を格納
  6) iterごと/全体の集計、best_solution.py を保存
  7) ★全iterの time_sec/peak_memory_kb/code_bytes の平均を overall_summary.json に出力
     （time の平均は TLE のケースを除外）

複数問題フォルダ（PROBLEM_DIRS）を「順番に」処理し、各問題配下に outputs/ を生成します。
"""
...
PROBLEM_DIRS: List[Path] = [
    Path("problems_llama/baseline/ABC421"),
    Path("problems_llama/baseline/ABC422"),
    # このように全問題を列挙
]
...
# Hugging Face トークン（環境変数 HF_TOKEN を優先）
