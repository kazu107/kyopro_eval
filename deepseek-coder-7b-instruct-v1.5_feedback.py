...
# -*- coding: utf-8 -*-
"""
[ローカル実行版・フィードバック付きパイプライン / 複数問題順次処理 / DeepSeek Coder 7B Instruct]

完全ローカル（オフライン）で deepseek-ai/deepseek-coder-7b-instruct-v1.5 を使って
「初回生成 + 最大N回のフィードバック改善」を 1 iter とし、各 iter の“最後のラウンド”だけを
overall_summary.json（pass@k互換）に記録します。TLE の time は平均から除外します。

前提:
  - モデル一式をローカルへ事前ダウンロード済み（例: D:\models\deepseek\deepseek-coder-7b-instruct-v1.5）
...
PROBLEM_DIRS: List[Path] = [
    Path("problems_deepseek/feedback/ABC392"),
]

# 生成（LLM）設定
N_TRIES = 100                 # iter（試行）回数
MAX_NEW_TOKENS = 800
TEMPERATURE = 0.7
TOP_P = 0.95
SEED_BASE = 42
MAX_FEEDBACK_ROUNDS = 3      # 追加のフィードバック回数（初回 + MAX_FEEDBACK_ROUNDS = 最大 4 ラウンド）
...
def process_problem(problem_dir: Path, tok: AutoTokenizer, model: AutoModelForCausalLM):
    ...
    # ---- ベスト選択（最後のラウンド結果に基づく）----
    def iter_key(x):
        # 優先: 全AC → passed 多い → rounds 少ない → 生成順（新しい優先）
        try:
